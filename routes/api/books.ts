...
import BookType from '../../models/types/BookType';
...
router.get('/', (req, res) => {
  Book.find()
    ...
    .catch((err: any) => res.status(404).json({ nobooksfound: 'No Books found' }));
});
...
router.delete('/:id', (req, res) => {
  Book.findByIdAndRemove(req.params.id, req.body)
    .then((book: BookType) => res.json({ mgs: 'Book entry deleted successfully' }))
